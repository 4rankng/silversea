...
- **Compose reality fix (`5e2b7359`)**: repo `deploy/docker-compose.prod.yml` still described a foreign project (nepocorp name/DB//opt paths) while the real server runs `name: vantai`, DB `vantai`, `/opt/vantai/data/*` — the divergence is why the first cutover stamped `buildHash:"dev"` (server compose predated the BUILD_HASH line; `make demo` ships images, not the compose file). Repo file now matches the server file + BUILD_HASH line; every nepocorp reference removed from tracked files (Makefile + dev-compose comments too). User directive: **do not touch anything nepocorp; staging is vantai.tingting.vip**.
- **Deploy verified**: cutover 17:17–17:25 UTC; rollback snapshot `/opt/vantai/.deploy-rollbacks/pre-cutover-20260912T171718Z.env`; DB backup `db-20260912T171747Z.dump` (1.1 MB); asset guard "asset guard OK"; public `/api/health` → `{"status":"ok","buildHash":"f8b186b1"}`; frontend + hashed CSS 200. Log `qa/2026-09-12_make-demo-deploy.log`.
- Trap for next session: server-side compose commands must use the repo compose file with the server's own project (`name: vantai` in `/opt/vantai/deploy/docker-compose.prod.yml`); a compose file with a different `name:` creates an orphan project on the same ports and fails with "port is already allocated". `make demo` is the safe path.
- During the deploy: Docker Desktop was upgraded/reinstalled by the user mid-session (app removed 23:57, engine back as 29.7.2); local dev containers (silversea-db/redis/adminer) were stopped to relieve VM memory and NOT restarted — restart with `make dev` when needed.

## Previous task — kanban wave: collapsible driver sections + design spec + deploy hardening — DONE (2026-09-12)

- Scope: the four "still needs implementation" items from the kanban build list — 20260911_2 BUG 1 (collapsible driver task info), 20260912_6 (design-consistency spec + verification), 20260912_4 items 2+8 (BUILD_HASH compose wiring, stale-asset deploy guard). No deploy (not authorized).
- **20260911_2 BUG 1 — collapsible sections** (`DriverTaskInfoSections.tsx` + `DriverTripDetailPage.css`): both fact sections ("Thông tin lệnh", "Thông tin xuất hóa đơn") get a full-head toggle (`aria-expanded`/`aria-controls`, chevron, ≥44px touch on coarse pointers); collapsed head keeps the factory short-name summary; independent per-section state, both start expanded. Unit: `DriverTripDetailPage.test.tsx` TC-COMP-004/004b. Live rung-3 (local dev, laixe, trip 8): `qa/2026-09-12_driver-detail-design/`. Card moved to kanban COMPLETED with evidence in the docx.
- **20260912_6 — design spec + verify** (`docs/driver-trip-detail-design-spec.md`): binding type-role table (24→18 screen title, 13 section titles, 14 body, 11 labels, 12 captions; sanctioned exceptions 16 step title + 15 seal evidence value), one-photo-block rule, two-tier buttons, banner treatment. Implementation (style wave `8daf4cfb`/`228e47e4`/`fe7c6dba`) verified against the ticket's 5 expectations; live font census + photo-block evidence in the same qa folder. Card moved to COMPLETED.
- **20260912_4 #2 — BUILD_HASH compose wiring** (`deploy/docker-compose.prod.yml`, `deploy/docker-compose.silversea.yml`, root `Makefile`): both server composes inject `BUILD_HASH=${BUILD_HASH:-dev}` into the backend env; `make demo`/`make deploy` stamp the cutover with the git short-sha (`DEPLOY_BUILD_HASH`) so `/api/health buildHash` names the deployed build. Note: root `docker-compose.dev.yml` has NO backend service (dev backend runs on host) — nothing to inject there.
- **20260912_4 #8 — stale-asset deploy guard** (`scripts/frontend-asset-guard.sh` + Makefile): post-cutover guard runs INSIDE the frontend container via `ssh ... docker exec -i <frontend> sh < script`; fails the cut if index.html references any missing hashed asset (fail-closed). Proven in a real nginx:alpine container (pass case exit 0, stale case exit 1). Server-side accumulation is already structurally impossible (assets are image-baked, containers recreated per cut); the browser-cache half was the no-cache index.html header shipped earlier today.
- Tree repair (uncommitted-damage recovery): the checkout had 614 tracked files deleted in the working tree (incl. workspace manifests + backend/Dockerfile + both compose files) plus a corrupt git object store and a stale `.git/index.lock` (still present — index-writing git ops blocked; commits this session may need `rm .git/index.lock` first). Restored the missing files from origin/prod (== local HEAD, verified same SHA) via a depth-1 clone + `rsync --ignore-existing` — modified files untouched; the 4 pre-existing modified files (backend build-hash, forwarder photos fix, q23 test, workboard test timeout) were preserved and remain UNCOMMITTED (other tickets' in-flight work).
- Gates: lint 0 errors (74 pre-existing warnings) · backend tsc 0 · frontend tsc 0 · frontend vitest **264 files / 1622 pass** (incl. 2 new) · `make build` green.
- Kanban: `20260911_2` + `20260912_6` → COMPLETED (evidence appended in docx); `20260912_4` stays TODO (items 1/4/5 open; #2+#8 marked done in docx).

**Updated:** 2026-09-12 21:25 Asia/Singapore
**Controller:** Claude (cook --auto kanban wave)
**Status:** DONE — local only, no push, no deploy.
