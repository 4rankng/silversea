...
| 0.3 | G-mig | per BE's staging-cut runbook (NOTES 13:45): via ssh psql, `__drizzle_migrations` carries 0067 prefix `2d8af75377` **AND** the MC-4 mini-cut's THREE migration sha256 prefixes: 0068_drop_governance_actions **`069b698197`**, 0069_shipment_finance_actions **`817007c7cd`**, 0070_salary_period_exclusions **`a8206d33b4`** (stamped 15:25 +07 from the user-session's working copies, recompute at cut if amended). **Key on the hash, not the row id** — staging ids run ahead of file numbers. ⚠️ 0068's Gate-B DO-block was REMOVED in the re-draft → zero-PENDING is now a MANUAL pre-cut step, not self-enforcing: run `SELECT COUNT(*) FROM governance_actions WHERE status LIKE 'PENDING%'` on the target env and confirm 0 BEFORE the cut. Re-entry check (0069/0070 no-carryover): staging's 3 legacy rows are all PRICE_CONFIG_CHANGE/APPLIED customer CREATEs (verified 15:24) → nothing to re-enter on staging; at the PROD cut, classify governance_actions rows by action_kind first — only shipment-finance-confirmation or salary-exclusion kinds would need re-entry. Missing any hash = **HARD STOP** via PM; never hand-patch | `2026-09-11_final-sweep_g-mig.log` |
