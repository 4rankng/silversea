...
| 0.3 | G-mig | per BE's staging-cut runbook (NOTES 13:45): via ssh psql, `__drizzle_migrations` carries 0067 prefix `2d8af75377` **AND** the new MC-4 migration sha256 prefix. **Key on the hash, not the row id** — staging ids run ahead of file numbers (`2d8af75377` sits at id 69, pre-cut baseline 14:04 +07). MC-4 file = `0068_drop_governance_actions.sql`, sha256 prefix **`fb51e9f5aa`** (stamped 14:07 +07 from FS's untracked working copy — recompute at cut time if amended; applies to the MC-4 follow-up cut, NOT the current MC-1-only cut). Missing either = **HARD STOP** via PM; never hand-patch | `2026-09-11_final-sweep_g-mig.log` |
