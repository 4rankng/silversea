...
def test_profit(ctx: NepoTestContext, results: TestResults):
    ...
    if '/profit' in page.url:
        ...
        results.fail_('TC-0501', 'Profit page', f'URL: {page.url}')
    ...
    if resp.get('status') == 200:
        ...
        results.fail_('TC-0502', 'Cap table API', f'Status: {resp.get("status")}')
    ...
    if resp.get('status') == 200:
        ...
        results.fail_('TC-0503', 'Distribution API', f'Status: {resp.get("status")}')
    ...
    if '/profit' in page.url:
        ...
        results.fail_('TC-0504', 'ACCOUNTANT profit', f'URL: {page.url}')
    page.close()
    ...
    if resp.get('status') == 403:
        ...
        results.fail_('TC-0505', 'DRIVER cap table', f'Expected 403, got {resp.get("status")}')
