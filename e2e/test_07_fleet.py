...
def test_fleet(ctx: NepoTestContext, results: TestResults):
    ...
    if '/dispatch' in page.url:
        ...
        results.fail_('TC-0701', 'Dispatch page', f'URL: {page.url}')
    ...
    if '/fleet' in page.url:
        ...
        results.fail_('TC-0702', 'Fleet page', f'URL: {page.url}')
    ...
    if resp.get('status') == 200:
        ...
        results.fail_('TC-0703', 'Trucks API', f'Status: {resp.get("status")}')
    ...
    if resp.get('status') == 200:
        ...
        results.fail_('TC-0704', 'Drivers API', f'Status: {resp.get("status")}')
    ...
    if resp.get('status') == 200:
        ...
        results.fail_('TC-0705', 'Trailers API', f'Status: {resp.get("status")}')
    ...
    if resp.get('status') in (200, 201) or resp.get('data', {}).get('id'):
        ...
        results.fail_('TC-0706', 'Create truck', f'Status: {resp.get("status")}')
