...
def test_debt(ctx: NepoTestContext, results: TestResults):
    ...
    if '/debt' in page.url:
        ...
        results.fail_('TC-0401', 'Debt list', f'URL: {page.url}')
    ...
    if resp.get('status') == 200:
        ...
        results.fail_('TC-0402', 'Receivables API', f'Status: {resp.get("status")}')
    ...
    if resp.get('status') == 200:
        ...
        results.fail_('TC-0403', 'Ledger API', f'Status: {resp.get("status")}')
    ...
    if cust_items:
        ...
        if stmt_resp.get('status') == 200:
            ...
            results.fail_('TC-0404', 'Customer statement', f'Status: {stmt_resp.get("status")}')
    else:
    ...
    if resp.get('status') == 403:
        ...
        results.fail_('TC-0405', 'DRIVER ledger access', f'Expected 403, got {resp.get("status")}')
