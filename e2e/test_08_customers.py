...
def test_customers(ctx: NepoTestContext, results: TestResults):
    api = ApiClient()
    ...
    if '/customers' in page.url:
        ...
        results.fail_('TC-0801', 'Customers page', f'URL: {page.url}')
    ...
    if resp.get('status') == 200:
        ...
        results.fail_('TC-0802', 'Customers API', f'Status: {resp.get("status")}')
    ...
    if resp.get('status') in (200, 201) or resp.get('data', {}).get('id'):
        ...
        results.fail_('TC-0803', 'Create customer', f'Status: {resp.get("status")}')
    ...
    if resp.get('status') == 200:
        ...
        results.fail_('TC-0804', 'ACCOUNTANT customers', f'Status: {resp.get("status")}')
    ...
    if resp.get('status') == 403:
        ...
        results.fail_('TC-0805', 'DRIVER customers', f'Expected 403, got {resp.get("status")}')
