...
Excel master-data delivery (staff, role accounts, drivers, customers, sites,
routes, ports, tractors, trailers, carriers).

Usage:
  python3 tools/generate-prod-master-data.py <Import data form.xlsx>

Since the 4.9 (2026-09-04) delivery the customer sends ONE workbook holding
every sheet (previously two files: "User & Role.xlsx" + "Data form.xlsx").
...
def extract_staff(wb):
    # "Nhân viên" (4.9 delivery): the real staff roster. No login column —
    # logins are derived from the full name (same convention as before) and
    # the sheet's own code (NTPHUONG) becomes employeeCode.
    for row in wb['Nhân viên'].iter_rows(min_row=3, values_only=True):
        code, name, role = cell(row[0]), cell(row[1]), cell(row[3])
...
def extract_role_accounts(wb):
    # "User" (4.9 delivery): shared role accounts with explicit logins
    # (admin/ketoan/dieuvan/cus). Names are the customer's own (truncated).
    for row in wb['User'].iter_rows(min_row=3, values_only=True):
        code, name, login, role_code, email = (cell(row[0]), cell(row[1]),
                                               cell(row[2]), cell(row[3]), cell(row[4]))
        if not login:
            continue
        role_accounts.append({
            'username': str(login).strip().lower(),
            'employeeCode': str(code),
            'fullName': str(name).strip(),
            'roleCode': str(role_code).strip(),
...
export interface ProdRoleAccountSeed { username: string; employeeCode: string; fullName: string; roleCode: string; email: string | null; }
...
def main():
    ...
    sections = [
        ...
        ('prodRoleAccounts', 'ProdRoleAccountSeed', role_accounts),
