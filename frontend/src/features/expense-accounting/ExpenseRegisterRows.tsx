...
export function ExpenseRegisterRows({ rows, selected, selectable, onSelect, onOpen, canViewPayments }: {
  ...
  return <div className="expense-register-table-wrap"><table className="expense-register-table">
    ...
    <tbody>{rows.map(entry => <tr key={expenseKey(entry)}>
      {selectable && <td className="expense-register-select"><input type="checkbox" aria-label={`Chọn ${entry.feeName} · ${entry.shipmentCode}`} checked={selected.has(expenseKey(entry))} disabled={entry.status !== 'RECORDED'} onChange={(event) => onSelect(entry, event.target.checked)} /></td>}
      <td data-label="Lô / công việc" className="expense-register-context"><button type="button" className="expense-register-open" onClick={() => onOpen(entry)}>{entry.shipmentCode}</button><span>{entry.customerName}</span><small>{entry.containerNumber ?? 'Phí chung lô'}{entry.tripCode ? ` · ${entry.tripCode}` : ''}</small>{entry.routeName && <small>{entry.routeName}</small>}</td>
