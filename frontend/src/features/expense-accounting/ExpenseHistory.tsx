...
import { ExpenseReconciliationHistory } from './ExpenseReconciliationHistory';
...
export function ExpenseHistory({ catalog, onVoucher }: { catalog: ExpenseAccountingCatalog; onVoucher: (entries: ExpenseAccountingEntry[], direction: 'IN' | 'OUT') => void }) {
  ...
  return <section className="expense-accounting" aria-label="Lịch sử thu chi và hoàn ứng">
    ...
    {view === 'reconciliations' ? <ExpenseReconciliationHistory canRelease people={catalog.opsUsers} onPay={item => item.remainingDifference < 0 ? setRefunding(item) : void pay(item)} /> : query.isError ? <p role="alert">Không tải được lịch sử. <button type="button" className="btn btn--secondary btn--sm" onClick={() => void query.refetch()}>Thử lại</button></p> : query.isPending ? <p role="status">Đang tải lịch sử…</p> : <div className="expense-accounting-history">
      {vouchers.data?.items.map(voucher => <article key={voucher.id}><header><button type="button" className="expense-register-open" onClick={() => setDetail(voucher)}>{voucher.code} · {voucher.direction === 'IN' ? 'Thu' : 'Chi'}</button><span>{voucher.status === 'REVERSED' ? 'Đã đảo' : 'Đã ghi tiền'}</span></header><dl><div><dt>Đối tượng</dt><dd>{voucher.counterpartyName ?? (voucher.counterpartyType === 'USER' ? catalog.staff.find(person => person.id === voucher.counterpartyId)?.name : voucher.counterpartyType === 'SUPPLIER' ? catalog.suppliers.find(person => person.id === voucher.counterpartyId)?.name : null) ?? 'Chưa có tên đối tượng'}</dd></div><div><dt>Quỹ / tài khoản</dt><dd>{voucher.treasuryAccountName ?? catalog.accounts.find(account => account.id === voucher.treasuryAccountId)?.name ?? 'Tài khoản đã ngừng dùng'}</dd></div><div><dt>Ngày</dt><dd>{formatDate(voucher.valueDate)}</dd></div><div><dt>Số tiền</dt><dd>{expenseMoney(voucher.amount)}</dd></div><div><dt>Tham chiếu</dt><dd>{voucher.physicalReference}</dd></div><div><dt>Phân bổ</dt><dd>{voucher.entries.length} khoản</dd></div></dl>{voucher.note && <p>{voucher.note}</p>}{voucher.status === 'RECORDED' && voucher.direction === 'IN' && voucher.counterpartyType === 'CUSTOMER' && (voucher.unappliedAmount ?? 0) > 0 && <div><p>Chưa phân bổ công nợ: {expenseMoney(voucher.unappliedAmount)}</p><button type="button" className="btn btn--secondary btn--sm" disabled={allocate.isPending} onClick={() => allocate.mutate(voucher)}>Phân bổ công nợ {voucher.code}</button></div>}{voucher.status === 'RECORDED' && <div><button type="button" className="btn btn--secondary btn--sm" onClick={() => setReversing(voucher)}>Đảo phiếu {voucher.code}</button></div>}</article>)}

      {!query.data?.items.length && <p className="expense-accounting-empty">Chưa có lịch sử.</p>}
    </div>}
    {detail && <ExpenseHistoryDetail item={detail} onClose={() => setDetail(null)} />}
