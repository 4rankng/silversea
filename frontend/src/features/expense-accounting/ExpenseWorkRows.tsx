...
export function ExpenseWorkRows({ rows, onOpen }: { rows: ExpenseWorkRow[]; onOpen: (row: ExpenseWorkRow, group: WorkFeeGroup) => void }) {
  return <div className="expense-register-table-wrap"><table className="expense-register-table expense-work-table">
    <thead><tr><th scope="col">Lịch / lô hàng</th><th scope="col">Khách / tuyến</th><th scope="col">Container / cảng</th><th scope="col">Điều phối</th><th scope="col">Ghi chú</th><th scope="col">Chi hộ phải thu</th><th scope="col">Chi hộ phải trả</th><th scope="col">Tiền đường</th></tr></thead>
    <tbody>{rows.map((row, index) => <tr key={row.id} data-vehicle-start={index > 0 && row.vehiclePlate !== rows[index - 1].vehiclePlate}>
      ...
      <td data-label="Khách / tuyến"><strong>{row.customerName}</strong><span>{row.routeName ?? 'Chưa có tuyến'}</span></td>
