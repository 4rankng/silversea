...
export function ShipmentFinanceForm({ editor, shipmentId, onClose, onSaved }: {
  editor: FinanceEditor; shipmentId?: number; onClose: () => void; onSaved: () => void;
}) {
  const formId = useId();
  ...
  async function save(event: React.FormEvent) {
    ...
    const parsed = editor.kind === 'invoice'
      ? shipmentInvoiceRecordSchema.safeParse({ ...identity, supplierId: Number(supplierId), invoiceNumber, invoiceDate,
        faceAmount, supplierFeeAmount: supplierFee, sourceExpenseId: Number(sourceExpenseId) || null })
  ...
  return <Drawer isOpen onClose={close} title={`${editor.record ? 'Cập nhật' : 'Thêm'} ${editor.kind === 'invoice' ? 'hóa đơn kết hợp' : 'cược container'}`}
    ...
    <form id={formId} className="shipment-finance__form" onSubmit={save}>
      ...
      {!shipmentId && !editor.record && <>
        ...
        <UuiSelectField label="Lô hàng" disabled={busy} required value={lot} onChange={(event) => { setLot(event.target.value); setSourceExpenseId(''); }}
      ...
      {editor.kind === 'invoice' ? <>
        ...
        <div className="shipment-finance__pair">
          <TextField controlSize="sm" label="Số Bill" required value={bill} maxLength={100} onChange={event => setBill(event.target.value)} disabled={busy} />
          <TextField controlSize="sm" label="Hãng tàu" required value={shippingLine} maxLength={255} onChange={event => setShippingLine(event.target.value)} disabled={busy} />
          <NumberField controlSize="sm" label="Tiền cược (đ)" value={amount} onChange={setAmount} min={1} step={1} max={999_999_999_999_999} required disabled={busy} />
          <DateField controlSize="sm" label="Ngày cược" value={depositDate} onChange={setDepositDate} required disabled={busy} />
