...
export default function TreasuryPositionPage() {
  ...
  return <div className="treasury-page">
    ...
    {data && <div className={`treasury-notice treasury-notice--${data.coverage.toLowerCase()}`}><strong>{COVERAGE[data.coverage]}</strong><span aria-hidden="true" className="treasury-notice__sep">·</span><span>Cập nhật đến {formatDateTimeVN(data.asOf)}. Đây là số dư ghi sổ, không phải số dư sao kê ngân hàng.</span></div>}
