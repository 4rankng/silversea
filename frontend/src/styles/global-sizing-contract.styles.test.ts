...
describe('global sizing contract', () => {
  ...
  it('promotes the operational type scale to :root so every page inherits it', () => {
    expect(opsCss).toMatch(/:root\s*\{[^}]*--ops-table-header-size:\s*11px/);
    expect(opsCss).toMatch(/:root\s*\{[^}]*--ops-table-primary-size:\s*13px/);
    expect(opsCss).toMatch(/:root\s*\{[^}]*--ops-table-supporting-size:\s*12px/);
    expect(opsCss).toMatch(/:root\s*\{[^}]*--ops-table-meta-size:\s*11px/);
  });
