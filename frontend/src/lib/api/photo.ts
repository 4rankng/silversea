 ...
 * `/api/photos/…` URL (as returned by a fresh OCR upload). Pending browser
 * `blob:` previews pass through unchanged. Bare keys are encoded so the
 * slashes survive as a single path segment that the wildcard photo route
 * decodes.
 */
