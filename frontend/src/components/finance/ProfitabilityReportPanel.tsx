...
export function ProfitabilityReportPanel({ month, year }: { month: number; year: number }) {
  ...
  return <section className="workflow-profitability" aria-labelledby="profitability-heading">
    ...
    {data && data.items.length === 0 && !loading && <div className="workflow-profitability__empty" role="status">
      <p>{lowMarginOnly ? 'Không có nhóm biên lợi nhuận thấp trong kỳ này.' : 'Chưa có dữ liệu lợi nhuận phù hợp trong kỳ này.'}</p>
      {lowMarginOnly && <button type="button" className="btn btn--secondary" onClick={() => { setLowMarginOnly(false); setPage(1); }}>Xem tất cả nhóm</button>}
    </div>}
