...
import { useBackShortcut } from './useBackShortcut';
import { subscribePagePopState } from '../lib/page-leave-events';
import { APP_NAVIGATION_EVENT, type AppNavigationDetail } from '../lib/app-navigation';

export function usePageLeaveGuard({ dirty, saving, confirm, onBack, message }: {
  message: string;
  dirty: boolean;
  saving: boolean;
  ...
  useEffect(() => {
    ...
    const unsubscribePopState = subscribePagePopState(historyBack);
    return () => {
