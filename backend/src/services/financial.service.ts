...
import { eq, and, inArray, sql, desc, isNull, gte, lte, ilike, or, count, sum, max } from 'drizzle-orm';
import { TxnType, type PenaltyStatus } from '@tingting/shared';
...
/**
 * Filters shared by the penalty list and summary reads. Dates are ISO
...
export interface PenaltySummaryFilters {
  driverId?: number;
  dateFrom?: string;
  dateTo?: string;
}

export interface PenaltySummaryDriverRow {
  driverId: number;
  driverName: string;
  /** Số lần vi phạm trong khoảng dateFrom..dateTo. */
  count: number;
  /** Tổng tiền phạt trong khoảng dateFrom..dateTo. */
  totalAmount: number;
  /** Số lần vi phạm từ đầu năm hiện tại. */
  ytdCount: number;
  /** Tổng tiền phạt từ đầu năm hiện tại. */
  ytdAmount: number;
  /** Ngày vi phạm gần nhất (toàn bộ lịch sử, không giới hạn khoảng lọc). */
  lastPenaltyDate: string | null;
  /** Số ngày liên tục không vi phạm — mirror computeStreak phía frontend. */
  streakDays: number;
}

export interface PenaltySummary {
  /** Tổng số biên bản trong khoảng lọc. */
  totalCount: number;
  /** Tổng tiền phạt trong khoảng lọc. */
  totalAmount: number;
  /** Số lượng theo trạng thái (ACTIVE/CANCELED) trong khoảng lọc. */
  statusCounts: Record<string, number>;
  /** Tổng tiền theo trạng thái trong khoảng lọc. */
  statusAmounts: Record<string, number>;
  /** Số lái xe có ít nhất 1 vi phạm trong khoảng lọc. */
  penalizedDriverCount: number;
  /** Số lần vi phạm từ đầu năm hiện tại (chỉ lọc theo driverId). */
  ytdCount: number;
  /** Tổng tiền phạt từ đầu năm hiện tại (chỉ lọc theo driverId). */
  ytdAmount: number;
  /** Phân tích theo lái xe, sắp xếp theo chuỗi an toàn giảm dần. */
  drivers: PenaltySummaryDriverRow[];
}
...
function buildPenaltyConditions(filters: {
  ...
  if (filters.status) conditions.push(eq(s.penalties.status, filters.status));
  if (filters.search) {
...
export async function getPenalties(filters: PenaltyListFilters = {}) {
  ...
  return { items, total: Number(countRows[0]?.total ?? 0), page, pageSize: limit };
}

/**
 * KPI aggregates over penalties — server-side replacement for the client-side
 * computation PenaltyTable used to run over the full list. Range figures honor
 * dateFrom/dateTo; YTD uses the current calendar year; streaks look at each
 * driver's whole history (matching the frontend computeStreak semantics).
 */
export async function getPenaltiesSummary(filters: PenaltySummaryFilters = {}): Promise<PenaltySummary> {
  const baseConds = [isNull(s.penalties.deletedAt)];
  if (filters.driverId) baseConds.push(eq(s.penalties.driverId, filters.driverId));

  const rangeConds = [...baseConds];
  if (filters.dateFrom) rangeConds.push(gte(s.penalties.date, filters.dateFrom));
  if (filters.dateTo) rangeConds.push(lte(s.penalties.date, filters.dateTo));

  const ytdConds = [...baseConds, gte(s.penalties.date, `${new Date().getFullYear()}-01-01`)];

  const driverConds = [isNull(s.drivers.deletedAt)];
  if (filters.driverId) driverConds.push(eq(s.drivers.id, filters.driverId));

  const [statusRows, rangeRows, ytdRows, lastRows, driverRows] = await Promise.all([
    db.select({ status: s.penalties.status, count: count(), amount: sum(s.penalties.amount) })
      .from(s.penalties).where(and(...rangeConds)).groupBy(s.penalties.status),
    db.select({ driverId: s.penalties.driverId, count: count(), amount: sum(s.penalties.amount) })
      .from(s.penalties).where(and(...rangeConds)).groupBy(s.penalties.driverId),
    db.select({ driverId: s.penalties.driverId, count: count(), amount: sum(s.penalties.amount) })
      .from(s.penalties).where(and(...ytdConds)).groupBy(s.penalties.driverId),
    db.select({ driverId: s.penalties.driverId, lastDate: max(s.penalties.date) })
      .from(s.penalties).where(and(...baseConds)).groupBy(s.penalties.driverId),
    db.select({ id: s.drivers.id, name: s.drivers.name, createdAt: s.drivers.createdAt })
      .from(s.drivers).where(and(...driverConds)),
  ]);
  ...
  const rangeByDriver = new Map(rangeRows.map((row) => [row.driverId, row]));
  ...
  const now = new Date();
  const drivers: PenaltySummaryDriverRow[] = driverRows.map((driver) => {
    ...
    return {
      ...
      count: rangeByDriver.get(driver.id)?.count ?? 0,
      totalAmount: Number(rangeByDriver.get(driver.id)?.amount ?? 0),
      ytdCount: ytdByDriver.get(driver.id)?.count ?? 0,
      ytdAmount: Number(ytdByDriver.get(driver.id)?.amount ?? 0),
      lastPenaltyDate,
      streakDays,
    };
  }).sort((a, b) => b.streakDays - a.streakDays || a.count - b.count);
  ...
  return {
    ...
    statusCounts,
    statusAmounts,
    penalizedDriverCount: rangeByDriver.size,
    ytdCount: ytdRows.reduce((acc, row) => acc + row.count, 0),
    ytdAmount: ytdRows.reduce((acc, row) => acc + Number(row.amount ?? 0), 0),
    drivers,
  };
}
