...
async function loadCustomerAdjustmentDomains(
  adjustmentRows: ReadonlyArray<{
    id: number;
    customerId: number;
    txnId: number | null;
...
): Promise<Map<number, OutstandingObligation['entityType']>> {
  const domains = new Map<number, OutstandingObligation['entityType']>();
  if (adjustmentRows.length === 0) return domains;

  const customerIds = [...new Set(adjustmentRows.map((row) => row.customerId))];
  const txnIds = [...new Set(adjustmentRows.flatMap((row) => (row.txnId == null ? [] : [row.txnId])))];
  const ledgerIds = adjustmentRows.map((row) => row.id);

  const tripAdjustmentRows = await db.select({
    ledgerEntryId: s.governanceActions.ledgerEntryId,
  })
    .from(s.governanceActions)
    .where(and(
      eq(s.governanceActions.actionKind, 'TRIP_AR_ADJUSTMENT'),
      inArray(s.governanceActions.ledgerEntryId, ledgerIds),
    ));
  const tripAdjustmentIds = new Set(
    tripAdjustmentRows.flatMap((row) => (row.ledgerEntryId == null ? [] : [row.ledgerEntryId])),
  );

  const tripRevenueRows = await db.select({
    customerId: s.ledger.entityId,
    txnId: s.ledger.txnId,
  })
    .from(s.ledger)
    .where(and(
      eq(s.ledger.entityType, 'CUSTOMER'),
      eq(s.ledger.txnType, 'TRIP_REVENUE'),
      inArray(s.ledger.entityId, customerIds),
      inArray(s.ledger.txnId, txnIds),
    ));
  const tripRevenueKeys = new Set(
    tripRevenueRows
      .filter((row) => row.txnId != null)
      .map((row) => `${row.customerId}:${row.txnId}`),
  );

  const serviceFeeRows = await db.select({
    customerId: s.ledger.entityId,
    txnId: s.ledger.txnId,
  })
    .from(s.ledger)
    .where(and(
      eq(s.ledger.entityType, 'CUSTOMER'),
      eq(s.ledger.txnType, 'SERVICE_FEE'),
      inArray(s.ledger.entityId, customerIds),
      inArray(s.ledger.txnId, txnIds),
    ));
  ...
  const billingDocumentRows = await db.select({
    ...
    .where(and(
      ...
      eq(s.billingDocuments.type, 'DEBIT_NOTE'),
      isNull(s.billingDocuments.deletedAt),
      inArray(s.billingDocuments.entityId, customerIds),
      inArray(s.billingDocuments.id, txnIds),
    ));
  const billingDocumentKeys = new Set(
    billingDocumentRows.map((row) => `${row.customerId}:${row.documentId}`),
  );

  const debtOffsetRows = await db.select({
    offsetId: s.debtOffsets.id,
    customerId: s.debtOffsets.customerId,
  })
    .from(s.debtOffsets)
    .where(and(
      inArray(s.debtOffsets.customerId, customerIds),
      inArray(s.debtOffsets.id, txnIds),
    ));
  const debtOffsetKeys = new Set(
    debtOffsetRows.map((row) => `${row.customerId}:${row.offsetId}`),
  );

  for (const row of adjustmentRows) {
    if (row.txnId == null) continue;
    const relationKey = `${row.customerId}:${row.txnId}`;
    if (tripAdjustmentIds.has(row.id)) {
      domains.set(row.id, 'TRIP');
      continue;
    }
    if (
      row.receiptId === `GBN:${row.txnId}`
      && billingDocumentKeys.has(relationKey)
    ) {
      domains.set(row.id, 'BILLING_DOCUMENT');
      continue;
    }
    if (
      isDebtOffsetAdjustmentRow(row.note, row.txnId)
      && debtOffsetKeys.has(relationKey)
    ) {
      domains.set(row.id, 'DEBT_OFFSET');
      continue;
    }
    if (
      isServiceFeeAdjustmentRow(row.note)
      && serviceFeeKeys.has(relationKey)
    ) {
      domains.set(row.id, 'SERVICE_FEE');
      continue;
    }
    if (tripRevenueKeys.has(relationKey)) {
      domains.set(row.id, 'TRIP');
    }
  }
...
function reminderObligationEntityTypeForTxnType(
  txnType: (typeof s.ledger.txnType.enumValues)[number],
): Exclude<OutstandingObligation['entityType'], 'DEBT_OFFSET'> | null {
  switch (txnType) {
    case 'SERVICE_FEE':
      return 'SERVICE_FEE';
    case 'ADJUSTMENT':
