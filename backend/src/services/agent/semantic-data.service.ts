...
const ENTITIES: Record<SemanticEntity, EntityDef> = {
  ...
  drivers: {
    ...
    from: sql`drivers d LEFT JOIN truck_driver_assignments tda ON tda.driver_id = d.id AND tda.ends_at IS NULL AND tda.role = 'PRIMARY' LEFT JOIN trucks tr ON tr.id = tda.truck_id`,
    fields: {
      id: number(sql`d.id`),
      name: text(sql`d.name`),
      phone: text(sql`d.phone`),
      assignedTruckId: number(sql`tda.truck_id`),
