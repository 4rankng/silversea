...
export async function operationsWorkInbox(userId: number, query: InboxQuery) {
  ...
  const items: OperationsWorkInboxItem[] = rows.map((row) => {
    ...
    const handoff = tripId != null && !exchangeIncomplete && !fact?.paperAt;
...
export async function adminHealth(query: InboxQuery) {
  ...
  else for (const job of failedJobs.value) items.push({ id: `health:job:${job.id}`, entityType: 'durable_effect_job', entityId: job.id, title: 'Tác vụ thông báo thất bại', subtitle: 'Cần kiểm tra hàng đợi bền vững', state: 'ACTION', priority: 100, dueAt: null, freshnessAt: job.updatedAt.toISOString(), blockers: emptyParty, advisories: emptyParty, nextAction: { label: 'Mở cấu hình', targetRoute: '/config' }, targetRoute: '/config', healthState: 'FAILED', source: 'durable_effect_jobs' });
  if (!failedEmail.ok) items.push(unavailable('customer_email_logs', 'Tình trạng email khách hàng'));
  else for (const email of failedEmail.value) items.push({ id: `health:email:${email.id}`, entityType: 'customer_email', entityId: email.id, title: 'Email khách hàng gửi thất bại', subtitle: 'Cần xử lý lại cấu hình hoặc hàng đợi', state: 'ACTION', priority: 90, dueAt: null, freshnessAt: email.updatedAt.toISOString(), blockers: emptyParty, advisories: emptyParty, nextAction: { label: 'Mở cấu hình', targetRoute: '/config' }, targetRoute: '/config', healthState: 'FAILED', source: 'customer_email_logs' });
  return pageWorkInboxItems(items, query);
}
