...
after(async () => {
  try {
    ...
    await db.delete(s.pricingTables).where(eq(s.pricingTables.customerId, createdCustomerIds[0] ?? 0));
    await db.delete(s.freightRateTerms).where(eq(s.freightRateTerms.customerId, createdCustomerIds[0] ?? 0));
