...
describe('container price-class selection at lot pricing (card 20260922_58)', () => {
  ...
  test('missing cargo weight at INTAKE: pricing is SKIPPED deterministically — no freeze, no invented price (ruling c)', async () => {
  ...
  test('missing weight at DISPATCH requires the 409 — silently unpriced dispatch is forbidden (QA rework)', async () => {
    const lot = await mkLot({ baseRateKey: 'CONT20', cargoWeightKg: null, cont20LightPrice: '3900000', cont20HeavyPrice: '4200000' });
    await assert.rejects(
      () => db.transaction((tx) => lockShipmentFreightRate(tx, {
    ...
    const snaps = await db.select({ id: s.freightRateSnapshots.id })
      .from(s.freightRateSnapshots).where(eq(s.freightRateSnapshots.shipmentId, lot.shipment.id));
    assert.equal(snaps.length, 0, 'no snapshot either — the block precedes any write');
  });
