...
const REVIEWED_NON_MATERIAL_MUTATIONS = new Map<string, string>([
  ...
  ['shipments/dispatch-planning.routes.ts|POST|/dispatch-task-tags', 'Reference-data CRUD (dispatch note-composer tag pool). Replay-safe by the normalized_label unique constraint — a duplicate POST returns 409 instead of duplicating; no financial or shipment-lifecycle mutation.'],
  ['shipments/dispatch-planning.routes.ts|PATCH|/dispatch-task-tags/:id', 'Reference-data CRUD (dispatch note-composer tag pool). Full-overwrite rename — replaying the same payload rewrites the identical label; a rename onto a key another row holds 409s through the normalized_label uniqueness check, so no durable command boundary is needed. No financial or shipment-lifecycle mutation.'],
  ['shipments/dispatch-planning.routes.ts|DELETE|/dispatch-task-tags/:id', 'Reference-data CRUD (dispatch note-composer tag pool). Soft-delete (isActive=false) keeping the unique normalized_label key — idempotent on replay (deactivating an inactive row still reports success); no financial or shipment-lifecycle mutation.'],
