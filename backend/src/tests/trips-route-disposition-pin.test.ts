...
// a trip whose assigned route no longer exists is removed — route_id is NOT
// NULL, so the stamp cannot be dropped, and a row whose stamp points nowhere
// would block the FK validation below. The live assignment survives
// untouched, and trips_route_id_fkey lands VALIDATED. The pin runs the
// file's own statements against a throwaway DB seeded with a schema-only
// dump of the dev database, with this one constraint dropped first to recreate the pre-migration state (the dev
...
before(async () => {
  ...
  // The file's own statements for this disposition, in file order. Prose
  // comment lines are stripped first so header notes never pollute the
  // statement extraction (the `-->` breakpoint markers are kept).
...
describe('trips route disposition', () => {
  test('a dangling route stamp removes its trip, the live assignment survives, and the constraint validates', async () => {
    const [counts] = await sql`
      select
        (select count(*)::int from trips) as "tripTotal",
        (select count(*)::int from trips where route_id is not null) as "assignedCount",
    ...
    assert.equal(counts.tripTotal, 1, 'the dangling trip is removed, nothing else');
    assert.equal(counts.assignedCount, 1, 'the live route assignment survives untouched');
