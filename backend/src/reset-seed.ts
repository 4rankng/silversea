...
export const WIPE_PLAN: Array<{ label: string; wipe: () => Promise<unknown> }> = [
  ...
  { label: 'freight_rate_terms', wipe: () => db.delete(s.freightRateTerms) },
  { label: 'fuel_norms', wipe: () => db.delete(s.fuelNorms) },
  { label: 'weight_pricing_tiers', wipe: () => db.delete(s.weightPricingTiers) },
