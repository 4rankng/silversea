...
const MATERIAL_WRITE_RULES: readonly MaterialWriteRule[] = [
  ...
  { method: 'POST', endpoint: 'ocr.fuel-evidence-reviews.decision', pattern: /^\/api\/ocr\/fuel-evidence-reviews\/[^/]+\/decision$/ },
  ...
  { method: 'POST', endpoint: IDEMPOTENCY_ENDPOINTS.CONTAINER_EDIT_REQUEST, pattern: /^\/api\/shipments\/cus-workspace\/[^/]+\/container-edit-request$/ },
  { method: 'POST', endpoint: IDEMPOTENCY_ENDPOINTS.CONTAINER_EDIT_REQUEST_DECISION, pattern: /^\/api\/shipments\/cus-workspace\/[^/]+\/container-edit-requests\/[^/]+\/decision$/ },
